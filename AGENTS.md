# Workora — guide for AI coding agents

**Workora** is an open-source, self-hosted alternative to Claude Tag — a Slack-style
...
- **`docs/MISSION.md`** — North star / working directive: what Workora is building
...
- **`docs/authorization.md`** — The authoritative access-control model: three auth planes, role→capability
  ...
  or anything that reads a resource by a client-supplied id. **Privilege escalation is dangerous.**
...
- Agent workspace lives at `~/.Workora/agents/<agent-id>/` with a `MEMORY.md` per agent. Optionally, a `personality.md` file in the same directory overrides the agent's `description` field — place an agency-agents personality file (e.g. from `~/.Workora/agency-agents/`) there to inject a detailed persona into the system prompt and MEMORY.md `## Role` section.
...
- Use `npm run wt:add -- <name>` to spin up an isolated git worktree (its own ports +
  `workora_<name>` database + redis index + **`OPEN_WORKORA_HOME=~/.Workora-<name>` data dir** +
  ...
  `ENV_FILE`; `src/paths.ts` reads `OPEN_WORKORA_HOME` — so each worktree is fully isolated.)
...
2. `cd ../Workora-<task>` and do the work.
...
> Prod / self-host machines run the compute-plane daemon as `npx @workora/Workora-daemon`
...
- A long-lived daemon keeps running the **old** bundle until **restarted** — bounce it
  (`npx @workora/Workora-daemon@latest`) on each prod machine after publishing.
...
- `OPEN_WORKORA_PRESSURE_MEM_MB` (default 500) — when free system memory drops below this (MB), new
