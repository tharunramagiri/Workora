...
export function Layout() {
  ...
  useEffect(() => { for (const v of ["--sb-w", "--traj-w"]) { const s = localStorage.getItem("Workora" + v); if (s) document.documentElement.style.setProperty(v, s); } }, []);
  ...
  const startResize = (which: "sb" | "traj") => (e: RMouseEvent) => {
    ...
    const onUp = () => { window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); localStorage.setItem("Workora" + varName, getComputedStyle(document.documentElement).getPropertyValue(varName).trim()); };
