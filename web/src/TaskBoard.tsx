...
export function TaskBoard({ channelId, onOpenThread }: { channelId: string | null; onOpenThread?: (t: Msg) => void }) {
  ...
  const [collapsed, setCollapsed] = useState<Set<string>>(() => { try { const s = localStorage.getItem("Workora.tasks.collapsed"); return new Set<string>(s ? JSON.parse(s) : ["done", "closed"]); } catch { return new Set(["done", "closed"]); } });
  const toggleCol = (k: string) => setCollapsed((c) => { const n = new Set(c); n.has(k) ? n.delete(k) : n.add(k); try { localStorage.setItem("Workora.tasks.collapsed", JSON.stringify([...n])); } catch { /* */ } return n; });
  // Board layout: horizontal Kanban columns (default) vs the legacy vertical stack; persisted like `collapsed`.
  const [boardLayout, setBoardLayout] = useState<"columns" | "stack">(() => { try { return localStorage.getItem("Workora.tasks.boardLayout") === "stack" ? "stack" : "columns"; } catch { return "columns"; } });
  const setLayout = (l: "columns" | "stack") => { setBoardLayout(l); try { localStorage.setItem("Workora.tasks.boardLayout", l); } catch { /* */ } };
