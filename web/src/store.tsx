...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const [slug, setSlug] = useState("Workora");
  ...
  const logout = () => { localStorage.removeItem(TOKEN_KEY); localStorage.removeItem("Workora.devuser"); window.location.assign("/login"); }; // clear token + dev user → redirect to login (JWT is short-lived; client-side removal is sufficient)
  ...
  useEffect(() => {
    ...
    setReady(false);
    sidRef.current = cur.id; setServerId(cur.id); setSlug(cur.slug || "Workora"); setMyRole(cur.role || "member"); setCapabilities(cur.capabilities || {});
