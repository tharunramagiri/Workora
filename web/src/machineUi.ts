...
export function daemonUpdateCommandTemplate(origin: string): string {
  return `npx @workora/Workora-daemon@latest --server-url ${origin} --api-key <your sk_machine_... key>`;
}
...
export function daemonConnectCommand(origin: string, key: string): string {
  return `npx @workora/Workora-daemon@latest --server-url ${origin} --api-key ${key}`;
}
