          ...
          <div className="scroll">
            <div className="card">
              {edit ? (
                ...
                <div className="kv"><b>{t("members.agentStateDirectory")}</b> ~/.Workora/agents/{a.id}</div>
...
function WorkspaceTab({ id }: { id: string }) {
  ...
  const [root, setRoot] = useState(`~/.Workora/agents/${id}/`); // shown in root bar + copied by copy button; fallback (old daemon/offline) replaced by the real on-disk path from the API
  useEffect(() => { setSel(null); setExpanded(new Set()); setRoot(`~/.Workora/agents/${id}/`); (async () => { const d = await api("GET", `/api/agents/${id}/workspace-files`); if (d.error) { setErr(d.error); setFiles([]); } else { setErr(""); setFiles(d.files || []); if (d.root) setRoot(d.root.endsWith("/") ? d.root : d.root + "/"); } })(); }, [id]);
...
// Description is visible to other humans and agents in the server; agents fetch it via `Workora server info` for collaboration context.
