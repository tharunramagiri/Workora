...
All notable changes to Workora are documented here.

Format: [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).
Versioning: these tags track the **compute-plane daemon** npm package
(`@workora/Workora-daemon`). The server and web app ship continuously
...
- **Per-agent project directory binding**: an owner can bind an inactive agent to an existing
  ...
  revalidates it before every start, and launches the runtime with it as `cwd`. Workora-owned
  memory, personality, notes, session artifacts, and reset scope remain under
  `OPEN_WORKORA_HOME/agents/<agent-id>`.
...
- A `project-browser-v1` machine-targeted RPC lets managers browse explicitly shared daemon roots
  and run bounded, marker-only project discovery from the Agent form. `OPEN_WORKORA_PROJECT_ROOTS` is the
...
- A Hermes startup turn that successfully checks an empty inbox now completes as a normal no-op.
  Previously its explanatory stdout was misclassified as `no-Workora-read`, suppressing the
...
- Agent detail responses now use an explicit serializer: environment variables and token/scope
  ...
  skills are read-only through Workora.
- Managed state reads/writes reject symbolic-link escapes, including generated runtime artifacts,
  ...
  redirect an Workora management action into the operator-owned project through a state symlink.
...
- **Version-skew NACK for unknown RPCs**: a daemon receiving an RPC `type` it doesn't know (a message
  ...
  it, so a ≥0.10.1 server can report "daemon too old — restart with `npx @workora/Workora-daemon@latest`"
...
- **Personality override**: place a `personality.md` file in the agent workspace
  (`~/.Workora/agents/<agent-id>/`) to inject a detailed persona into the system
...
- `.cmd` wrapper: `ensureWorkoraBin()` generates `Workora.cmd` on Windows
...
- Workspace tab shows the real daemon workspace root when `OPEN_WORKORA_HOME` is
...
- **Agent task handoff**: `Workora task assign --message-id <id> --to @agent`
...
- Remote daemons (daemon host ≠ server host) injected
  `OPEN_WORKORA_SERVER_URL=http://localhost:PORT` into spawned agents, so every
  `Workora` CLI call failed with `fetch failed`. The daemon now spawns agents
...
- Self-contained esbuild bundle — daemon process + agent CLI — runnable with
  `npx @workora/Workora-daemon --server-url <url> --api-key sk_machine_…`
  on any machine with Node ≥ 20, without cloning the repository.
- Supported runtimes at time of release: **Claude Code** and **Codex**.

[Unreleased]: https://github.com/workora/Workora/compare/v0.10.0...HEAD
[0.10.0]: https://github.com/workora/Workora/compare/v0.9.1...v0.10.0
[0.9.1]: https://github.com/workora/Workora/compare/v0.9.0...v0.9.1
[0.9.0]: https://github.com/workora/Workora/compare/v0.8.3...v0.9.0
[0.8.3]: https://github.com/workora/Workora/compare/v0.8.2...v0.8.3
[0.8.2]: https://github.com/workora/Workora/compare/v0.8.1...v0.8.2
[0.8.1]: https://github.com/workora/Workora/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/workora/Workora/compare/v0.7.1...v0.8.0
[0.7.1]: https://github.com/workora/Workora/compare/v0.7.0...v0.7.1
[0.7.0]: https://github.com/workora/Workora/compare/v0.6.1...v0.7.0
[0.6.1]: https://github.com/workora/Workora/compare/v0.6.0...v0.6.1
[0.6.0]: https://github.com/workora/Workora/compare/v0.5.1...v0.6.0
[0.5.1]: https://github.com/workora/Workora/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/workora/Workora/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/workora/Workora/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/workora/Workora/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/workora/Workora/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/workora/Workora/releases/tag/v0.1.0
