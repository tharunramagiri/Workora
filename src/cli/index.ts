#!/usr/bin/env node
// Workora CLI — agent-side communication layer. runtimes invoke it via their bash tools.
// Auth/routing via env vars injected by daemon at spawn time:
//   OPEN_WORKORA_SERVER_URL, OPEN_WORKORA_AGENT_TOKEN (per-agent token, injected by daemon), OPEN_WORKORA_AGENT_ID (or --agent-id)
...
const BASE = process.env.OPEN_WORKORA_SERVER_URL ?? "http://localhost:7777";
const KEY = process.env.OPEN_WORKORA_AGENT_TOKEN ?? process.env.OPEN_WORKORA_MACHINE_KEY ?? process.env.OPEN_WORKORA_API_KEY ?? "poc-secret-key"; // per-agent token takes priority (slice10)
const AGENT = process.env.OPEN_WORKORA_AGENT_ID ?? "";
const TURN_FILE = process.env.OPEN_WORKORA_TURN_FILE ?? "";
...
program.name("Workora").description("Workora agent CLI").version("0.1.0");
...
action.command("prepare").description("prepare an action card (action JSON from stdin; variants: channel:create / agent:create)")
  .requiredOption("--target <ch>", "#channel / dm:@name").action(async (opts) => {
    ...
    if (!raw) { console.error("Error: action JSON required on stdin"); console.error('Next action: echo \'{"type":"channel:create","name":"x","description":"…"}\' | Workora action prepare --target "#general"'); process.exit(1); }
...
// project: agent-side git workflow for imported repos (buzz-style: agents push work directly).
// Requires the agent to be bound to a project (OPEN_WORKORA_PROJECT_PATH set by the daemon).
const project = program.command("project").description("git workflow for the bound project (imported repo)");
project.command("push").description("commit all changes and push to a feature branch on the project's remote")
  ...
  .action(async (opts) => {
    ...
    const message = opts.message || "workora: agent changes";
    const { execFile } = await import("node:child_process");
    const { promisify } = await import("node:util");
    const exec = promisify(execFile);
    ...
    if (opts.createChannel) {
      ...
      } catch { console.log("Channel creation skipped (no project binding on server)."); }
    }
  });
project.command("branches").description("list branches on the bound project").action(async () => {
  ...
  try {
    ...
    console.log(out);
  } catch (e: any) { console.error(`git branch failed: ${e?.stderr ?? e?.message ?? e}`); process.exit(1); }
});
