...
const IDLE_MS = Number(process.env.OPEN_WORKORA_IDLE_MS ?? 10 * 60 * 1000); // how long before idle sleep (kills process to save memory; next wake uses --resume)
const DELIVER_DEBOUNCE_MS = Number(process.env.OPEN_WORKORA_DELIVER_DEBOUNCE_MS ?? 3000); // batching window for deliveries while agent is busy (saves tokens, reduces interruptions)
const ONE_SHOT_DELIVER_DEBOUNCE_MS = Number(process.env.OPEN_WORKORA_ONE_SHOT_DELIVER_DEBOUNCE_MS ?? process.env.OPEN_WORKORA_HERMES_DELIVER_DEBOUNCE_MS ?? 500); // One-shot runtimes need a short fixed wait when there is only one live notice.
const PENDING_DELIVER_TTL_MS = Number(process.env.OPEN_WORKORA_PENDING_DELIVER_TTL_MS ?? 15_000); // start+deliver can arrive back-to-back; keep deliver briefly while start prepares workspace
