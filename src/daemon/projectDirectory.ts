...
function parseRootInputs(raw = process.env.OPEN_WORKORA_PROJECT_ROOTS): string[] {
  const value = raw?.trim();
  if (!value) return [];
  let inputs: unknown;
  if (value.startsWith("[")) {
    try { inputs = JSON.parse(value); }
    catch { throw new ProjectDirectoryError("project_roots_not_configured", "OPEN_WORKORA_PROJECT_ROOTS is invalid JSON"); }
  ...
  catch { throw new ProjectDirectoryError("project_roots_not_configured", "OPEN_WORKORA_PROJECT_ROOTS must contain absolute paths"); }
}
