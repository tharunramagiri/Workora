...
test("Cursor uses a state-managed always-on plugin on fresh and resumed turns", () => {
  const stateDir = mkdtempSync(path.join(tmpdir(), "Workora-cursor-state-"));
  try {
    const pluginDir = prepareCursorPlugin(stateDir, "Workora prompt");
    const manifest = JSON.parse(readFileSync(path.join(pluginDir, ".cursor-plugin", "plugin.json"), "utf8"));
    const rule = readFileSync(path.join(pluginDir, "rules", "Workora.mdc"), "utf8");
    assert.match(manifest.name, /^Workora-/);
    assert.match(rule, /alwaysApply: true/);
    assert.match(rule, /Workora prompt/);
