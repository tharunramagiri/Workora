// Hermes runtime: one-shot `hermes chat -q <prompt> -Q --source Workora` per turn.
//
// Hermes owns provider credentials and profile configuration. Workora only selects a Hermes profile
// (temporarily stored in agent.model) and passes the Workora system prompt + message into an isolated
// agent workspace. This keeps secrets in Hermes config, not in the Workora database.
...
export function buildHermesArgs(prompt: string, sessionId?: string | null): string[] {
  const args = ["chat", "-q", prompt, "-Q", "--source", "Workora"];
...
class HermesRun {
  ...
  private runTurn(input: HermesInput): void {
    ...
    const turnFile = path.join(tmpdir(), `Workora-hermes-turn-${Date.now()}-${Math.random().toString(36).slice(2)}.jsonl`);
    const proc = spawnSafe("hermes", args, { cwd: this.opts.cwd, stdio: ["ignore", "pipe", "pipe"], env: { ...this.env, OPEN_WORKORA_TURN_FILE: turnFile } });
    this.proc = proc;
