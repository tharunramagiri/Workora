...
test("Hermes CLI args use quiet chat mode for Workora", () => {
  assert.deepEqual(buildHermesArgs("hello"), ["chat", "-q", "hello", "-Q", "--source", "Workora"]);
});

test("Hermes CLI args resume the captured native Hermes session", () => {
  assert.deepEqual(buildHermesArgs("hello", "20260702_221211_1991f1"), ["chat", "-q", "hello", "-Q", "--source", "Workora", "--resume", "20260702_221211_1991f1"]);
});
...
test("Hermes prompt carries Workora system prompt, project cwd, state dir, and user message", () => {
  const prompt = buildHermesPrompt("please help", { cwd: "/tmp/project", stateDir: "/tmp/Workora-agent", systemPrompt: "use Workora cli" });
  assert.match(prompt, /operator-selected project directory: \/tmp\/project/);
  assert.match(prompt, /Workora-owned state directory is: \/tmp\/Workora-agent/);
  assert.match(prompt, /use Workora cli/);
  assert.match(prompt, /please help/);
});

test("Hermes profile discovery reads profile dirs with default first, then alphabetical", () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-hermes-"));
  try {
...
test("Hermes profile home resolves named profiles without changing global defaults", () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-hermes-home-"));
  try {
...
test("Hermes runtime default profile clears inherited profile env", () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-hermes-env-"));
  try {
...
test("Hermes runtime resolves profiles from HERMES_PROFILE_DIR as well as ~/.hermes/profiles", () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-hermes-profile-dir-"));
  try {
...
test("Hermes rejects initial admission exactly once when its argv process cannot spawn", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-hermes-missing-"));
...
test("Hermes completes an empty-inbox turn so a queued delivery can run", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-hermes-empty-inbox-"));
  const binDir = path.join(root, "bin");
  mkdirSync(binDir);
  if (process.platform === "win32") {
    writeFileSync(path.join(binDir, "hermes.cmd"), [
      "@echo off",
      "echo {\"type\":\"check\",\"count\":0}>>\"%OPEN_WORKORA_TURN_FILE%\"",
      "echo No new Workora messages were waiting. Standing by.",
      "exit /b 0",
    ].join("\r\n"), "utf8");
  } else {
    writeFileSync(path.join(binDir, "hermes"), [
      "#!/bin/sh",
      "printf '%s\\n' '{\"type\":\"check\",\"count\":0}' >> \"$OPEN_WORKORA_TURN_FILE\"",
      "printf '%s\\n' 'No new Workora messages were waiting. Standing by.'",
      "exit 0",
    ].join("\n"), { mode: 0o755 });
  }
