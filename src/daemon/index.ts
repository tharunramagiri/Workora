#!/usr/bin/env node
// Workora local daemon: connects to the control-plane WS and spawns locally-installed CLI agents (claude/codex) on demand.
// Usage: Workora-daemon --server-url http://localhost:7777 --api-key <machineKey>
...
const DELIVERY_COMMIT_TIMEOUT_MS = Math.max(2_000, Number(process.env.OPEN_WORKORA_DELIVERY_COMMIT_TIMEOUT_MS ?? 15_000));
...
// Stable machine identity: on first connection the server assigns machine.id via ready:ack, persisted to ~/.Workora/machine-id.
...
conn = new Connection(serverUrl, apiKey, (msg) => {
  ...
  switch (msg.type) {
    ...
    case "git:branches": void listBranches(String(msg.clonePath ?? "")).then(
    ...
    default:
      ...
      if (typeof msg.requestId === "string" && msg.requestId) conn.send({ type: "rpc:nack", requestId: msg.requestId, error: `daemon ${process.env.DAEMON_VERSION ?? "dev"} does not support "${msg.type}" — restart it with: curl -fsSL https://raw.githubusercontent.com/tharunramagiri/Workora/main/scripts/install-daemon.sh | bash -s --` });
  }
}, () => {
...
log.info("Workora daemon starting", { serverUrl });
conn.connect();
