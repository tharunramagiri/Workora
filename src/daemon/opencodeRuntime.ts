...
// handleOpencodeEvent maps one parsed `opencode run --format json` event to Workora callbacks.
...
export function opencodeInstructionEnv(env: NodeJS.ProcessEnv, stateDir: string, systemPrompt: string): { env: NodeJS.ProcessEnv; agentName: string } {
  ...
  if (raw) {
    ...
    catch { throw new Error("OPENCODE_CONFIG_CONTENT must be valid JSON before Workora can append instructions"); }
