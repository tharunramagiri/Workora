...
test("stop resolves only after the stopped runtime reports exit", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-stop-settle-"));
...
test("reset does not wipe the workspace until the old runtime reports exit", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-reset-settle-"));
  let callbacks!: RuntimeCallbacks;
  const fakeRuntime: Runtime = {
    name: "fake",
    start(_opts, cb) {
...
test("start rejects when the runtime exits before initial turn admission", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-start-admission-"));
  const fakeRuntime: Runtime = {
    name: "fake",
    start(_opts, cb) {
...
test("a late exit callback from an old runtime cannot delete a replacement instance", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-exit-owner-"));
...
test("deliver received during async start is consumed by the wake nudge, not re-delivered as a notice", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-"));
  ...
  try {
    ...
    assert.match(initialPrompt ?? "", /Workora message check/);
...
test("durable delivery waits for the server commit barrier before touching a hot runtime", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-commit-"));
  const delivered: string[] = [];
...
test("a rejected server commit creates no runtime work or Activity preview", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-commit-reject-"));
  const sent: any[] = [];
  let deliveries = 0;
  const fakeRuntime: Runtime = {
    name: "fake",
...
test("durable preparations preserve arrival order even when storage lookup could reorder", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-order-"));
...
test("one-shot runtime start with pending delivery uses wake nudge without a second notice", async () => {
  ...
  try {
    ...
    assert.match(initialPrompt ?? "", /Workora message check/);
    assert.match(initialPrompt ?? "", /Workora message send/);
    assert.equal(delivered.length, 0);
    mgr.stopAll();
  } finally {
    rmSync(root, { recursive: true, force: true });
...
test("reset waits for cancelled startup workspace I/O before wiping the workspace", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-reset-wipe-"));
  let runtimeStarts = 0;
  const fakeRuntime: Runtime = {
    name: "fake",
    start() {
...
test("reset waits for cancelled startup workspace I/O before writing the final cleared memory", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-reset-memory-"));
  const fakeRuntime: Runtime = {
    name: "fake",
...
test("reset rejects when workspace cleanup fails instead of acknowledging a false success", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-reset-error-"));
...
test("clearMemory atomically replaces a MEMORY.md symlink without touching its target", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-reset-memory-link-"));
...
test("syncProfile never reads or writes through a MEMORY.md symlink", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-profile-memory-link-"));
...
test("start rejects a symlinked agent state directory before writing or spawning", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-state-link-"));
  const outside = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-state-link-outside-"));
...
test("a bound project is runtime cwd while state and full reset remain isolated", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-project-"));
  ...
  const previousRoots = process.env.OPEN_WORKORA_PROJECT_ROOTS;
  process.env.OPEN_WORKORA_PROJECT_ROOTS = JSON.stringify([root]);
  const fakeRuntime: Runtime = {
    name: "fake",
    start(opts, cb) {
      startOpts = opts;
  ...
  try {
    ...
    if (previousRoots === undefined) delete process.env.OPEN_WORKORA_PROJECT_ROOTS;
    else process.env.OPEN_WORKORA_PROJECT_ROOTS = previousRoots;
    rmSync(root, { recursive: true, force: true });
  }
});

test("a missing bound project fails before spawning the runtime", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-agent-manager-missing-project-"));
  const previousRoots = process.env.OPEN_WORKORA_PROJECT_ROOTS;
  process.env.OPEN_WORKORA_PROJECT_ROOTS = JSON.stringify([root]);
  let starts = 0;
  const fakeRuntime: Runtime = {
