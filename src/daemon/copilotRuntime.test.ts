...
// This is the repo's first test — Workora otherwise verifies via typecheck + real-run E2E. node:test
...
test("managed instructions append to Copilot's existing instruction directories without touching the project", () => {
  const stateDir = mkdtempSync(path.join(tmpdir(), "Workora-copilot-state-"));
  try {
    const env = copilotInstructionEnv({ COPILOT_CUSTOM_INSTRUCTIONS_DIRS: "/existing,/shared,/existing" }, stateDir, "Workora prompt");
    const dirs = env.COPILOT_CUSTOM_INSTRUCTIONS_DIRS!.split(",");
    assert.deepEqual(dirs.slice(0, 2), ["/existing", "/shared"]);
    assert.equal(dirs.length, 3);
    assert.equal(readFileSync(path.join(dirs[2]!, "AGENTS.md"), "utf8"), "Workora prompt");
  } finally {
