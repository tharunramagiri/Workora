...
test("managed runtime artifacts stay below the agent state directory", () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-runtime-artifact-"));
  try {
...
test("managed runtime artifacts reject a symlinked parent directory", () => {
  const root = mkdtempSync(path.join(tmpdir(), "Workora-runtime-artifact-link-"));
  const outside = mkdtempSync(path.join(tmpdir(), "Workora-runtime-artifact-outside-"));
  try {
