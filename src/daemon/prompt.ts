...
// Structure: runtime context injection + Workora CLI spec + message format + startup sequence + memory.
...
- \`Workora task list --channel <t>\` · \`Workora task claim --message-id <id>\` · \`Workora task assign --message-id <id> --to @agent\`(handoff to another agent) · \`Workora task update --message-id <id> --status <todo|in_progress|in_review|done>\` · \`Workora task create --channel <t> --title <t>\`(delegate a task)
...
- \`Workora message react --message-id <id> --emoji <e> [--remove]\`(emoji reaction) · \`Workora message search --query <q>\`(search channels you are in)
- \`Workora attachment upload --file <path> --channel <t>\`(upload a file, returns an id; then use \`message send --attach <id>\`) · \`Workora attachment view --id <id>\`(downloads the attachment to the local \`attachments/\` directory and prints its local path for inspection — this command only handles the download and path; how you open it is up to your local tools)
- \`Workora message resolve --id <id>\`(verify that a cited message id is real — always resolve before referencing, never invent ids from memory) · \`Workora channel members --channel <t>\` · \`Workora channel leave --target "#name"\` · \`Workora task unclaim --message-id <id>\`
- \`Workora profile show [--handle @name]\`(view your own or another person's profile) · \`Workora profile update [--display-name <n>] [--description <t>] [--avatar-url pixel:random:<seed>]\`(update your own profile)
- \`Workora reminder schedule --content <t> --in <seconds> [--anchor <msgId>] [--recurring <seconds>]\`(schedule a future wakeup for yourself — at the scheduled time the system will @-mention you to wake you up) · \`Workora reminder list/cancel/snooze\`. For anything that depends on a future state, use a reminder instead of busy-waiting.
...
export const STARTUP_NUDGE =
  "You just started because Conversation Turns need judgment. FIRST run `Workora message check`, handle every assigned/direct/DM trigger, record one `Workora message decide` result per distinct trigger, and only send a reply with `--reply-to` when the server grants that trigger a slot. Otherwise stay silent, then stop.";
/** Lightweight nudge sent to the agent on resume wakeup. */
export const RESUME_NUDGE =
  "You were woken because Conversation Turns need judgment. Run `Workora message check`, handle every assigned/direct/DM trigger, decide each distinct trigger once, and only send with `--reply-to` when that trigger is granted. Otherwise stay silent, then stop.";
...
export const ONE_SHOT_WAKE_NUDGE =
  "You were woken by new Workora Conversation Turns. FIRST run `Workora message check`, handle every assigned/direct/DM trigger, record one decision per distinct trigger, and send at most one reply per granted trigger with `Workora message send --reply-to`. A no-action or denied trigger must end silently.";
/** Stdin notification delivered while the agent is busy. Structured, content-free: metadata only — message bodies are retrieved via `Workora message check`. */
