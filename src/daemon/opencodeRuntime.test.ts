...
test("OpenCode merges a managed primary agent into existing config content", () => {
  ...
  const configured = opencodeInstructionEnv({ OPENCODE_CONFIG_CONTENT: existing }, "/state/Agent One", "Workora prompt");
  ...
  assert.equal(parsed.agent[configured.agentName].prompt, "Workora prompt");
});
