...
async function onDaemon(ws: WebSocket, key: string): Promise<void> {
  ...
  if (safeEqual(key, BOOTSTRAP_KEY)) {
    serverId = (await db.select().from(schema.servers).where(eq(schema.servers.slug, "Workora")))[0]?.id ?? null;
  } else {
  ...
  frames.open(async (data) => {
    ...
    try {
      ...
      else if ((msg.type === "workspace:file_tree" || msg.type === "workspace:file_content" || msg.type === "workspace:file_write" || msg.type === "workspace:file_delete" || msg.type === "skills:list" || msg.type === "project:resolved" || msg.type === "project:directories" || msg.type === "models" || msg.type === "agent:resource-budget" || msg.type === "rpc:ack" || msg.type === "rpc:nack" || msg.type === "git:cloned" || msg.type === "git:status" || msg.type === "git:pulled" || msg.type === "git:pushed" || msg.type === "git:projects-root" || msg.type === "git:branches" || msg.type === "git:checkout" || msg.type === "git:tested" || msg.type === "git:diff" || msg.type === "skills:written") && msg.requestId) resolveDaemonRequest(msg.requestId, msg, ws);
