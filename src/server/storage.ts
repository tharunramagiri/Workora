...
// env OPEN_WORKORA_STORAGE=local|s3; s3 uses @aws-sdk/client-s3 to connect to any S3-compatible endpoint (OPEN_WORKORA_S3_*).
...
// ── S3-compatible driver (disabled by default; enable by running `npm i @aws-sdk/client-s3` and setting OPEN_WORKORA_S3_ENDPOINT/BUCKET/KEY/SECRET, optional OPEN_WORKORA_S3_REGION) ──
...
export function s3Config(): S3Config {
  const endpoint = process.env.OPEN_WORKORA_S3_ENDPOINT;
  const bucket = process.env.OPEN_WORKORA_S3_BUCKET;
  const key = process.env.OPEN_WORKORA_S3_KEY;
  const secret = process.env.OPEN_WORKORA_S3_SECRET;
  const missing = ([
    ["OPEN_WORKORA_S3_ENDPOINT", endpoint], ["OPEN_WORKORA_S3_BUCKET", bucket],
    ["OPEN_WORKORA_S3_KEY", key], ["OPEN_WORKORA_S3_SECRET", secret],
  ] as const).filter(([, v]) => !v).map(([n]) => n);
  if (missing.length) throw new Error(
    `OPEN_WORKORA_STORAGE=s3 requires ${missing.join(", ")} to be set (see README → Object storage).`);
  return { endpoint: endpoint!, region: process.env.OPEN_WORKORA_S3_REGION ?? "us-east-1", bucket: bucket!, key: key!, secret: secret! };
}
