...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (url.pathname === "/health") return sendJson(res, 200, { ok: true, service: "Workora", time: new Date().toISOString() });
...
reconcileCounters()
  ...
  .finally(() => server.listen(PORT, () => {
    log.info("control plane up", { url: `http://localhost:${PORT}`, logs: "~/.Workora/logs/" });
