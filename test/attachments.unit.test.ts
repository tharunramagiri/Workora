...
test("parseUpload rejects (not hangs/crashes) when saveObject fails before consuming the stream", { timeout: 8000 }, async () => {
  ...
  process.env.OPEN_WORKORA_STORAGE = "s3";
  process.env.OPEN_WORKORA_S3_ENDPOINT = "http://127.0.0.1:9000";
  process.env.OPEN_WORKORA_S3_KEY = "k";
  process.env.OPEN_WORKORA_S3_SECRET = "s";
  delete process.env.OPEN_WORKORA_S3_BUCKET; // missing → s3Config() throws "requires OPEN_WORKORA_S3_BUCKET"
  ...
  await assert.rejects(parseUpload(req), /OPEN_WORKORA_S3_BUCKET/);
});
