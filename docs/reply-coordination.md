...
14. Primary publication waits up to `OPEN_WORKORA_REPLY_SETTLE_MS` (default 5000 ms) from
...
```text
Workora message decide --message-id <id> --decision no_action
Workora message decide --message-id <id> --decision request_reply \
  --reason better_fit --summary "I own humor responses"
Workora message decide --message-id <id> --decision delegate --to @codex2
Workora message decide --message-id <id> --decision abstain
Workora message send --reply-to <id> --target <target>
```
