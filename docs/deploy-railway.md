# Deploying Workora to Railway (control plane)

This guide deploys the Workora **control plane** (web + API + Postgres + Redis) to
Railway and binds `getworkora.com`. The daemon runs on your own machine and connects
...
```
Workora-web           ← this repo root Dockerfile
  └─ NODE_ENV=production
...
(daemon)               ← YOUR machine, not Railway
  └─ npx @workora/Workora-daemon --server-url https://www.getworkora.com …
```
...
2. **New Project → Empty Project**. Name it `Workora`.
...
- Select `workora/Workora`, branch `main`.
...
Open the **`Workora-web`** service → **Variables** tab. Add every variable below.
...
2. Click **Create bucket**. Name it `Workora` (or `Workora-prod`).
...
In the **`Workora-web`** service → **Variables**, add:

| Variable | Value |
|---|---|
| `OPEN_WORKORA_STORAGE` | `s3` |
| `OPEN_WORKORA_S3_ENDPOINT` | `https://<ACCOUNT_ID>.r2.cloudflarestorage.com` |
| `OPEN_WORKORA_S3_BUCKET` | `Workora` (the bucket name from 5a) |
| `OPEN_WORKORA_S3_KEY` | *(R2 Access Key ID from 5b)* |
| `OPEN_WORKORA_S3_SECRET` | *(R2 Secret Access Key from 5b)* |
| `OPEN_WORKORA_S3_REGION` | `auto` |
...
4. If it is: R2 is working. If not: check `OPEN_WORKORA_STORAGE` is set to `s3` and
...
## Step 7 — Custom domain (`getworkora.com`)

### 7a — Add the domain in Railway

1. Open the **`Workora-web`** service → **Settings → Networking → Custom Domain**.
2. Add `www.getworkora.com`. Railway shows a **CNAME target** (e.g.,
   `Workora-web.up.railway.app` or similar — copy it exactly).
3. Optionally add `getworkora.com` (the apex) as a second custom domain. Railway will
...
### 7b — DNS records in Aliyun Cloud DNS

Go to [Aliyun DNS console](https://dns.console.aliyun.com/) → select `getworkora.com`.

#### For `www.getworkora.com` (subdomain — standard CNAME)

| Field | Value |
|---|---|
| Record type | `CNAME` |
| Host | `www` |
| Value | `<Railway CNAME target for www.getworkora.com>` |
| TTL | `600` (10 minutes; change to 3600 once stable) |

#### For `getworkora.com` (apex domain)
...
Add a "URL redirect" record to redirect the apex to `www`:

| Field | Value |
|---|---|
| Record type | `URL redirect` (301 redirect; hidden forwarding does not expose the domain) |
| Host |  |
| Value |  |

> **CRITICAL**: URL redirect only works for browser HTTP(S). It does **not** forward
> WebSocket or API traffic. Always use `https://www.getworkora.com` (not the bare apex)
> for: daemon `--server-url`, API calls, Socket.IO connections. Share the `www` URL
> with users — their browsers will still see `getworkora.com` due to the redirect, but
...
1. Add `getworkora.com` to Cloudflare (free plan).
2. In the Aliyun domain registrar, update the nameservers to Cloudflare's.
...
With this setup, both `getworkora.com` and `www.getworkora.com` work for
...
Verify:
```bash
curl -I https://www.getworkora.com/health
...
npx @workora/Workora-daemon@latest \
  --server-url https://www.getworkora.com \
  --api-key <DAEMON_BOOTSTRAP_KEY>
```

Use `https://www.getworkora.com` (the `www` subdomain), not the bare apex, to ensure
...
- R2 is not configured. Complete Step 5. Without R2, `OPEN_WORKORA_STORAGE` defaults to
...
- DNS has not propagated yet. Run `dig www.getworkora.com CNAME` to check. If the
...
| `OPEN_WORKORA_STORAGE` | Set to `s3` for R2 — see Step 5 |
| `OPEN_WORKORA_S3_REGION` | Use `auto` for Cloudflare R2 (not `us-east-1`) |
