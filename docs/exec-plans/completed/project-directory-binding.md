...
runtime-native project context remain available. Workora's own durable state remains under
`OPEN_WORKORA_HOME/agents/<agent-id>` and reset/delete operations never delete or rewrite the bound
project.
...
- `stateDir` and `projectDir` are distinct. `stateDir` owns `MEMORY.md`, `notes/`, personality,
  ...
  that Workora only uses as cwd and as a source of runtime-native project context.
...
- Existing `AGENTS.md`, `CLAUDE.md`, `.cursor/rules`, `.mcp.json`, and similar project files are never
  overwritten, permanently appended, renamed, or deleted by Workora.
- "Append Workora instructions" means protocol-level composition: use the runtime's system,
...
| Runtime | Project cwd | Workora instruction composition | Project files touched |
|---|---|---|---|
...
| Hermes | spawn `cwd` | existing per-turn Workora prompt envelope | none |
...
- 2026-07-28: created isolated worktree `Workora-project-directory` from `origin/main` at
...
- Bind the directory per agent, not per Workora server/workspace: filesystem paths are local to one
...
- Do not implement literal permanent append to repository instruction files. Concurrent agents would
  accumulate contradictory identities and crash recovery could leave stale Workora policy inside a
  user's source tree.
