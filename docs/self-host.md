# Self-Hosting Workora

> **Windows deployment**: see [`docs/self-host-windows.md`](./self-host-windows.md)
> for running Workora on Windows with Docker Desktop, Caddy HTTPS, and Task Scheduler
> daemon management.
>
...
This guide walks you through running Workora on your own infrastructure — from a
fresh Ubuntu VPS to a long-running production stack.

## Architecture overview

Workora has two planes you need to run:
...
(`DATABASE_URL=postgres://…@postgres:5432/workora` and `REDIS_URL=redis://redis:6379`).
Leave those as-is.
...
export OPEN_WORKORA_PROJECT_ROOTS='["/srv/projects"]'
...
```bash
export OPEN_WORKORA_PROJECT_ROOTS='["/absolute/path/to/projects"]'
npx @workora/Workora-daemon@latest \
...
> The daemon stores agent workspaces, `MEMORY.md` files, and logs under `OPEN_WORKORA_HOME`
> (default: `~/.Workora`). See [Data directory](#data-directory-workora_home) below.
> `OPEN_WORKORA_PROJECT_ROOTS` is separate: it controls which host directories may be browsed or
...
```nginx
# /etc/nginx/sites-available/Workora
server {
    listen 80;
...
```bash
ln -s /etc/nginx/sites-available/Workora /etc/nginx/sites-enabled/
...
## Data directory (`OPEN_WORKORA_HOME`)
...
~/.Workora/
  ...
  bin/Workora              # generated CLI wrapper injected into agents' PATH
  logs/                     # server and daemon logs
  uploads/                  # attachments (when using local storage)
```

`OPEN_WORKORA_HOME` defaults to `~/.Workora`. Override it with the environment variable:

```bash
export OPEN_WORKORA_HOME=/data/Workora
```
...
## Shared project roots (`OPEN_WORKORA_PROJECT_ROOTS`)
...
export OPEN_WORKORA_PROJECT_ROOTS='["/srv/projects","/home/agent/code"]'
...
system directories. `OPEN_WORKORA_PROJECT_ROOTS` is a disclosure/binding policy, **not a runtime sandbox**:
...
`OPEN_WORKORA_HOME/uploads/` on the container's filesystem. Since the container is stateless
...
volumes:
  - uploads:/home/node/.Workora/uploads
```
...
| Data | Location | How |
|---|---|---|
| Database (all messages, tasks, agents, etc.) | Postgres `workora` database | `pg_dump` |
| Agent workspaces + `MEMORY.md` | `$OPEN_WORKORA_HOME/agents/` on the daemon host | `rsync` / `tar` |
| Uploads (if local storage) | `$OPEN_WORKORA_HOME/uploads/` on the server host | `rsync` / `tar` |
...
docker exec Workora-pg \
  pg_dump -U workora -d workora -Fc \
  > "Workora-db-$(date +%Y%m%d-%H%M%S).dump"
```
...
cat > /usr/local/bin/Workora-backup.sh << 'SCRIPT'
#!/bin/bash
set -euo pipefail
DEST="/backups/Workora-db-$(date +%Y%m%d-%H%M%S).dump"
docker exec Workora-pg pg_dump -U workora -d workora -Fc > "$DEST"
find /backups -name 'Workora-db-*.dump' -mtime +14 -delete
SCRIPT
sudo chmod +x /usr/local/bin/Workora-backup.sh
```

```cron
# /etc/cron.d/Workora-backup  (one line — no backslash continuation)
0 3 * * * root /usr/local/bin/Workora-backup.sh
```
...
docker exec -i Workora-pg \
  pg_restore -U workora -d workora --clean --if-exists \
  < Workora-db-YYYYMMDD-HHMMSS.dump
...
tar -czf "Workora-agents-$(date +%Y%m%d).tar.gz" ~/.Workora/agents/
```
...
```env
DATABASE_URL=postgres://workora:workora@localhost:5433/workora
...
# OPEN_WORKORA_HOME=/data/Workora
```
...
sudo cp docs/systemd/Workora-server.service /etc/systemd/system/
sudo cp docs/systemd/Workora-daemon.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now Workora-server
sudo systemctl enable --now Workora-daemon
```
...
sudo systemctl status Workora-daemon
journalctl -u Workora-server -f
journalctl -u Workora-daemon -f
```

The server logs also land in `$OPEN_WORKORA_HOME/logs/` (default `~/.Workora/logs/`).
...
sudo systemctl restart Workora-server
...
npm show @workora/Workora-daemon version

# To update, restart the daemon — npx will pull the latest bundle:
# (for systemd)
sudo systemctl restart Workora-daemon
...
> (`@workora/Workora-daemon`). A merged server change does **not** automatically
...
| `OPEN_WORKORA_HOME` | no | `~/.Workora` | Base dir for agent workspaces, logs, uploads |
| `OPEN_WORKORA_PROJECT_ROOTS` | for project binding | unset (fail closed) | JSON array or platform PATH-delimited daemon-host roots exposed to the project picker and accepted as project cwd |
| `OPEN_WORKORA_UPLOAD_DIR` | no | `$OPEN_WORKORA_HOME/uploads` | Override local upload directory |
| `OPEN_WORKORA_STORAGE` | no | `local` | `local` or `s3` for S3-compatible object storage |
| `OPEN_WORKORA_S3_ENDPOINT` | if `s3` | — | S3 endpoint URL |
| `OPEN_WORKORA_S3_BUCKET` | if `s3` | — | Bucket name |
| `OPEN_WORKORA_S3_KEY` | if `s3` | — | Access key |
| `OPEN_WORKORA_S3_SECRET` | if `s3` | — | Secret key |
| `OPEN_WORKORA_S3_REGION` | no | `us-east-1` | S3 region |
| `OPEN_WORKORA_IDLE_MS` | no | `600000` | Agent idle-sleep timeout in ms (default 10 min) |
| `OPEN_WORKORA_TURN_DEBOUNCE_MS` | no | `1200` | Human/agent ambient burst window in ms; scoped independently per sender and concrete channel (max 30000) |
| `OPEN_WORKORA_DIRECT_TURN_DEBOUNCE_MS` | no | `800` | Explicit-mention/DM burst window in ms (max 30000); task/action boundaries dispatch immediately |
| `OPEN_WORKORA_TURN_MAX_WAIT_MS` | no | `5000` | Hard cap from the first message to Turn dispatch, even when a sender keeps extending the trailing quiet window (max 30000) |
| `OPEN_WORKORA_REPLY_SETTLE_MS` | no | `5000` | Reply-intent settlement window, measured from grant activation after Turn dispatch |
| `OPEN_WORKORA_AGENT_DELIVERY_ACK_MS` | no | `2000` | Wait for daemon admission ACK/NACK before retrying the same deterministic Turn delivery id. A capable daemon emits pending heartbeats while a busy runtime queue still owns the delivery; those renew this timeout but do not count as ACK. For persistent/protocol runtimes final ACK means stdin/protocol acceptance; for argv one-shot runtimes it means the child process spawned, not model completion. Resource-pressure queueing stays pending. |
| `OPEN_WORKORA_DELIVERY_PENDING_HEARTBEAT_MS` | no | `750` | Daemon heartbeat interval while a Turn delivery is still waiting for runtime admission (minimum 250 ms). Must remain below `OPEN_WORKORA_AGENT_DELIVERY_ACK_MS`; heartbeat is transport liveness, never final admission. |
| `OPEN_WORKORA_DELIVERY_COMMIT_TIMEOUT_MS` | no | `15000` | Maximum wait for the server's durable `agent:deliver:admitted` response before the daemon NACKs without writing runtime input. |
