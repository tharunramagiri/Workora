...
Deploy the Workora control plane to Railway as a production-shaped preproduction
...
machine to the public control-plane WebSocket, preserving Workora's three-plane
architecture.
...
`REDIS_URL=${{Redis.REDIS_URL}}` to `Workora-web`.
...
and a configurable `OPEN_WORKORA_S3_ENDPOINT`, compatible with Cloudflare R2. Set
`OPEN_WORKORA_S3_REGION=auto` for R2.

Before any public use, complete Step 5 of `docs/deploy-railway.md` (create R2 bucket,
set `OPEN_WORKORA_STORAGE=s3` + `OPEN_WORKORA_S3_*` variables in Railway) and verify
...
1. The Railway Web service is connected to
   `https://github.com/ramagiritharun/Workora`, branch `main`.
...
5. Admin login succeeds and loads the seeded `Workora` workspace.
