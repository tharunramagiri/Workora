# Design — GitHub Copilot CLI runtime for Workora
...
**Chosen: (A) pipe mode, spawn one process per turn.** Rejected (B) `copilot --acp` (needs a whole ACP transport Workora lacks; out of scope). (A) fits Workora's sleep/wake-by-resume model cleanly: no persistent process to babysit; "resume on wake" is the same resume every turn uses.
...
Copilot is Workora's **first one-shot/per-turn runtime** — a test that the `Runtime` contract absorbs this shape unchanged.
...
| Copilot event (v1.0.61) | Workora action |
|---|---|
