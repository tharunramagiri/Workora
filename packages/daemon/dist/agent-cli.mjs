...
var BASE = process.env.OPEN_WORKORA_SERVER_URL ?? "http://localhost:7777";
var KEY = process.env.OPEN_WORKORA_AGENT_TOKEN ?? process.env.OPEN_WORKORA_MACHINE_KEY ?? process.env.OPEN_WORKORA_API_KEY ?? "poc-secret-key";
var AGENT = process.env.OPEN_WORKORA_AGENT_ID ?? "";
var TURN_FILE = process.env.OPEN_WORKORA_TURN_FILE ?? "";
function headers() {
...
program2.name("Workora").description("Workora agent CLI").version("0.1.0");
...
message.command("check").description("non-blocking check for new messages").action(async () => {
  ...
  if (!messages.length && !updates.length) {
    await recordTurnEvent({ type: "check", count: 0 });
    return console.log("No new messages.");
  }
  const targets = Array.from(new Set([
    ...messages.map((m) => targetFromText(String(m.text ?? ""))),
    ...updates.map((u) => u.target)
  ].filter(Boolean)));
  const granted = [...messages].reverse().find((m) => m?.coordination?.grantStatus === "active");
...
message.command("decide").description("record whether and why this agent should reply to an observed message").requiredOption("--message-id <id>", "trigger message full/short id").requiredOption("--decision <decision>", "no_action | request_reply | accept | delegate | abstain").option("--reason <reason>", "ownership | better_fit | handoff | correction | blocker | new_evidence | unique_expertise").option("--summary <text>", "short evidence for a reply request").option("--to <agent>", "delegate target handle").action(async (opts) => {
...
action.command("prepare").description("prepare an action card (action JSON from stdin; variants: channel:create / agent:create)").requiredOption("--target <ch>", "#channel / dm:@name").action(async (opts) => {
  ...
  if (!raw) {
    ...
    console.error(`Next action: echo '{"type":"channel:create","name":"x","description":"\u2026"}' | Workora action prepare --target "#general"`);
    process.exit(1);
  }
  let actionObj;
  try {
...
var checkpoint = program2.command("checkpoint").description("save session context to git history (workora/checkpoints/v1 branch)");
checkpoint.command("save").description("write session JSON (from stdin) into the checkpoints branch").option("--message <text>", "short label for this checkpoint (default: session checkpoint)").action(async (opts) => {
  const raw = (await readStdin()).trim();
  if (!raw) {
    console.error("Error: session JSON required on stdin");
    console.error(`Next action: echo '{"session":"\u2026","toolCalls":[\u2026]}' | Workora checkpoint save`);
    process.exit(1);
  }
  ...
  const run = async (args) => {
    try {
      ...
      console.error(`git ${args[0]} failed: ${e?.stderr ?? e?.message ?? e}`);
      process.exit(1);
    }
  };
  ...
  await mkdir2(join2(projectPath, dir), { recursive: true });
  await writeFile2(join2(projectPath, dir, "session.json"), raw);
  await writeFile2(join2(projectPath, dir, "meta.json"), JSON.stringify({ agent: AGENT, message: opts.message ?? "session checkpoint", at: (/* @__PURE__ */ new Date()).toISOString() }, null, 2));
  await run(["add", dir]);
...
checkpoint.command("resume").description("list recent checkpoints for this project (read from the checkpoints branch)").action(async () => {
  const projectPath = process.env.OPEN_WORKORA_PROJECT_PATH ?? "";
  if (!projectPath) {
